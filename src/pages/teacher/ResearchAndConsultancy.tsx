...
const ResearchAndConsultancy: React.FC = () => {
  ...
  const fetchEntries = async () => {
    ...
    try {
      const q = query(
        collection(db, "ResearchConsultancy"),
  ...
  const addOrUpdateEntry = async (
    ...
    try {
      if (entryId) {
        ...
        const entryRef = doc(db, "ResearchConsultancy", entryId);
        ...
        // Add new entry
        await addDoc(collection(db, "ResearchConsultancy"), newEntry);
        await logActivity(
          "Add",
          "ResearchConsultancy",
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", padding: "20px" }}>
      <b><h1 style={{fontSize: "38px",marginBottom: "20px"}}>🔍 Research and Consultancy</h1></b>
...
const Card: React.FC<{
  ...
  const handleDelete = async (entryId: string, title: string, category: string) => {
    try {
      await deleteDoc(doc(db, "ResearchConsultancy", entryId));
      await logActivity(
        "Delete",
        "ResearchConsultancy",
